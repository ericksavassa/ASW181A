namespace RegrasNegocio
{
    ...
    public class PersistenciaFuncionario : IPersistenciaFuncionario
    {
        ...
        public Funcionario BuscaFuncionario(string id)
        {
            ...
            var funcionarios = (List<Funcionario>)Access.Query<Funcionario>("SELECT * FROM PESSOA WHERE Id = " + id);
            if (funcionarios != null)
                funcionario = funcionarios.FirstOrDefault();
            return funcionario;
        }

        public int AlteraFuncionario(int id, string ativo)
        {
            Log.WriteLog("    Alterando status do funcionario");
            var inicioGrava = DateTime.Now;
            
            var result = Access.Execute(@"UPDATE PESSOA SET ATIVO = @ATIVO
                                    WHERE ID = @ID", new { ativo, id });
